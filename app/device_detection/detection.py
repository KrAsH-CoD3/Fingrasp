...
def _get_platform_group(name: str | None) -> str:
    ...
    if any(
        ...
        return "android"
    if "mac" in name_lower or any(
        k in name_lower for k in ("apple m1", "apple m2", "apple m3", "apple m4")
    ):
        return "mac"
...
def validate_device_model(
    ...
    detected_name = (
        extract_device_name(fingerprint) if fingerprint else "Unknown Device"
    )
    ...
    if (
        not user_model
        or user_model_lower == "not_sure"
        or user_model_lower == "unknown device"
    ):
        logger.info(
            f"User selected 'Not Sure' or provided no model. Detected: {detected_name}"
        )
    ...
    if (
        user_group != "unknown"
        and detected_group != "unknown"
        and user_group != detected_group
    ):
        logger.warning(
            f"Platform mismatch! User claimed '{user_model}' ({user_group}), "
    ...
    is_android = bool(
        exact_from_fingerprint
        or user_group == "android"
        or detected_group == "android"
        or any(
            k in user_model_lower
            for k in ("samsung", "pixel", "galaxy", "sm-", "android")
        )
    )
    ...
    if is_android:
        if exact_from_fingerprint:
            logger.info(
                f"Android exact model found from fingerprint store lookup: '{exact_from_fingerprint}'. Overriding user input '{user_model}'."
            )
            ...
            if identified:
                logger.info(
                    f"Android user input '{user_model}' validated against store as '{identified}'."
                )
                return identified

            # Still unrecognized? Tag it with the hardware reality from signals
            logger.info(
                f"Unrecognized Android model '{user_model}'. Tagging with hardware profile."
            )
    ...
    if detected_group in ("ios", "mac"):
        ...
        if detected_group == "ios" and fingerprint:
            # Use _get_screen_dims for consistency with extract_device_name
            screen_dims = _get_screen_dims(fingerprint)
            if not screen_dims:
                screen_dims = (0, 0)
            w, h = screen_dims
            ...
            if valid_models and user_model not in valid_models:
                logger.warning(
                    f"Hardware mismatch! User claims {user_model}, but screen key {screen_key} only supports {valid_models}"
                )
                return f"{user_model} ({detected_name})"
        ...
        is_consistent = any(
            variant in user_model_lower or user_model_lower in variant
        ...
        if not is_consistent and detected_name not in (
            ...
            logger.warning(
                f"Consistency check failed on {detected_group}! User: '{user_model}', Hardware Profile: '{detected_name}'."
            )
            return f"{user_model} ({detected_name})"
    ...
    logger.info(
        f"User provided model '{user_model}' matching detected platform '{detected_group}'."
    )
    return user_model
